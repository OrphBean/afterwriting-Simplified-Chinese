/**
 * Chinese Fountain Parser - Helpers Module
 * Provides token manipulation and query utilities for Chinese screenplay parsing
 */
...
operators.name = function() {
    ...
    var pFull = character.indexOf("（");
    if (p !== -1) {
        character = character.substring(0, p);
    } else if (pFull !== -1) {
        character = character.substring(0, pFull);
    }
...
operators.location = function() {
    ...
    // Remove Chinese scene heading prefixes: 内景, 外景, 内外景, 内/外景
    location = location.replace(/^(内景\/外景|内外景|内\/外景|内景|外景)/, "");
    // Handle both - and full-width dash
    var dash = location.lastIndexOf(" - ");
    if (dash === -1) {
        dash = location.lastIndexOf(" － "); // full-width dash
    }
    if (dash !== -1) {
...
operators.has_scene_time = function(time) {
    ...
    if (!suffix || suffix.length === 0) {
        suffix = this.text.substring(this.text.indexOf(" － ")); // full-width dash
    }
...
operators.location_type = function() {
    var location = this.text.trim();
    if (/^(内外景|内\/外景)/.test(location)) {
        return "mixed";
    }
    else if (/^内景/.test(location)) {
        return "int";
    }
    else if (/^外景/.test(location)) {
        return "ext";
    }
    return "other";
};
